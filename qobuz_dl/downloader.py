"""Album / track download logic for qobuz-dl."""
...
def _quality_chain(cfg: Dict[str, Any], start_qid: str) -> List[str]:
    """Ordered quality ids to try, highest first, starting at start_qid."""
    ...
    # only start from start_qid and lower
    if start_qid in ids:
...
def _embed(
    ...
    if path.suffix.lower() == ".mp3":
        embed_mp3_metadata(
            path, track, cover, meta_fields, force_main_album_artist, override_main_artist
        )
    else:
        embed_flac_metadata(
            path, track, cover, meta_fields, force_main_album_artist, override_main_artist
...
def download_single_track(
    ...
    track_no = track.get("track_number") or track.get("track_number") or ""
    ...
    fname = truncate_name(fname, cfg, "track") if hasattr(truncate_name, "__call__") else fname
    # truncate_name signature in utils expects (name, cfg, kind)
    try:
    ...
    for qid in chain:
        ...
        try:
            ...
            # try other tokens if API supports
            ok_url = False
        ...
        if _download_file(api.session, url, dest, progress, retries):
            if cfg.get("embed_metadata", True):
                _embed(
                    ...
                    cover,
                    meta_fields,
                    force_main_album_artist,
                    override_main_artist,
                )
...
def download_album(
    ...
    """Download full album. Returns artist_id string if useful for auto-override."""
    try:
    ...
    if cfg.get("save_cover", True) or cfg.get("embed_metadata", True):
        try:
            if cfg.get("save_cover", True):
                cover_bytes = fetch_cover(
                    album, api.session, cfg.get("cover_size", "original")
                )
                if cover_bytes:
            ...
            cover_for_embed = fetch_cover_for_embed(
                album,
                api.session,
                cfg.get("embed_cover_size", "large"),
    ...
    with Progress(
        ...
        for tr in tracks:
            ...
            if download_single_track(
                ...
                cover=cover_for_embed or cover_bytes,
